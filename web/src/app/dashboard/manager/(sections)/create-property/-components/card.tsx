...
export const CreatedPropertyDetailsCard = ({className, propertyDetails, ...props}: CreatedPropertyDetailsCardProps) => {
    const {
        name,
        description,
        pricePerMonth,
    ...
    const firstPhoto = photoUrls && photoUrls.length > 0 ? photoUrls.find(Boolean) ?? null : null

    return (
        <Card className={[className ?? "", "max-w-2xl"].join(" ").trim()} {...props}>
            <div className="flex flex-col md:flex-row gap-4">
                <div className="md:w-48 w-full h-40 bg-gray-100 rounded-md overflow-hidden flex-shrink-0">
                    {firstPhoto ? (
                        <img src={firstPhoto} alt={name ?? "property photo"} className="w-full h-full object-cover" />
                    ) : (
                ...
                <div className="flex-1">
                    <div className="flex items-start justify-between">
                        ...
                        <div className="text-right">
                            <div className="text-lg font-medium">{formatCurrency(pricePerMonth)}/mo</div>
                            <div className="text-sm text-gray-500">Deposit: {formatCurrency(securityDeposit)}</div>
                        </div>
                    </div>
                    ...
                    <div className="mt-3 text-sm">
                        ...
                        <div className="text-gray-700">
                            {averageRatings?.toFixed?.(1) ?? "—"} ({numberOfRatings ?? 0} reviews)
                        </div>
                    </div>
                    ...
                    <div className="mt-3 text-sm text-gray-600">
                        ...
                        <div>Payment applications: {propertyTenantPaymentApplicationIds?.length ?? 0}</div>
                    </div>
                </div>
            </div>
        </Card>
